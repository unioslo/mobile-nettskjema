...
class NettskjemaCsrfTokenFactory implements CsrfTokenFactory {
    ...
    @Override
    public MultipartRequestField newCsrfToken(final String token) {
        return new MultipartRequestField() {
            @Override
            public void addToBuilder(MultipartBody.Builder bodyBuilder) {
                bodyBuilder.addFormDataPart("NETTSKJEMA_CSRF_PREVENTION", token);
            }
        };
    }
